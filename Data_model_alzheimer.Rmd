...
```{r}
pollution_alz_recipe <- recipe(log_alzheimers ~ ., data = pollution_training) %>% 
  step_rm(County,
          Year,
          All.causes..total.,
          #Alzheimer.s.disease,
...
pollution_final_stack %>% 
  ...
  geom_abline(slope = 1, 
              intercept = 0, 
              color = "darkred") +
  labs(x = "Actual log(alzheimers)", 
       y = "Predicted log(alzheimers)")
```
