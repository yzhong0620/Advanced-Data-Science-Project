...
pm25 <- rbind(pm2017, pm2018, pm2019) %>% 
  ...
  summarise(mean_pm25 = mean(`Daily Mean PM2.5 Concentration`))
```
...
data %>% 
  ...
  ggplot(aes(x = mean_pm25, y = asthma_er_avg)) +
  geom_point() +
...
data %>% 
  filter(!(COUNTY == "California")) %>% 
  ggplot(aes(x = mean_pm25, y = cancer_incidence_rate)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "Cancer")
```
...
california_deaths_2017 <- california_deaths %>% 
  ...
  group_by(County, Strata_Name, Cause, Cause_Desc) %>% 
  summarize(deaths_per_year = mean(Count))
```

```{r}
california_deaths_edited <- california_deaths_2017 %>% 
  group_by(Cause_Desc) %>% 
