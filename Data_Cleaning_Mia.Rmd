...
```{r}
data_deaths_no_pop <- california_deaths_all_years %>% 
  #filter(Cause == "ALL") %>% 
...
```{r}
data_deaths <- data_deaths_no_pop %>% 
  left_join(ca_pop, by = c("County" = "County", "Year" = "Year"))
```

```{r}
data_standardized <- data_deaths %>% 
  mutate(total_deaths_pp = `All causes (total)`/Population,
         alzheimers_deaths_pp = `Alzheimer's disease`/Population,
         malignant_neoplasm_deaths_pp = `Malignant neoplasms`/Population,
         respiratory_deaths_pp = `Chronic lower respiratory diseases`/Population)
```
